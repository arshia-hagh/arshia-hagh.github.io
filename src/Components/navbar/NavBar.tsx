...
function NavBar() {
  ...
  return (
    <header className={`bg-primery--color sticky top-0 ${flag ? 'translate-y-[-100%]' : 'translate-y-0'} transition-transform z-[100]  w-screen flex items-center p-6 justify-between`} >
      ...
      <div className="rounded border-[2px] w-20 relative  flex   border-[#c86305]">
        ...
        <span  onClick={() => i18n.changeLanguage('fa')} className="cursor-pointer  text-center  w-full ">FA</span>
        <span onClick={() => i18n.changeLanguage('en')} className="cursor-pointer   text-center w-full">EN</span>
      </div>
      </div>
